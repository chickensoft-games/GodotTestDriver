...
public class TabContainerDriverTest : DriverTest
{
  ...
  {
    _tabContainer = new TabContainerDriver(
      () => RootNode.GetNode<TabContainer>("TabContainer")
    );
    _firstTabContent = new ControlDriver<Control>(
      () => _tabContainer.Root?.FindChild("First Tab Content") as Control
    );
    _secondTabContent = new ControlDriver<Control>(
      () => _tabContainer.Root?.FindChild("Second Tab Content") as Control
    );
  }
