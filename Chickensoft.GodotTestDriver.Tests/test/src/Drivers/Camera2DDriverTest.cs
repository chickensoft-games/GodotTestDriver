...
public class Camera2DDriverTest : DriverTest
{
  ...
  {
    _camera2D = new Camera2DDriver(
      () => RootNode.GetNode<Camera2D>("Camera2D")
    );
    _centerSprite = new Sprite2DDriver(
      () => RootNode.GetNode<Sprite2D>("CenterSprite")
    );
    _offCenterSprite = new Sprite2DDriver(
      () => RootNode.GetNode<Sprite2D>("OffCenterSprite")
    );
  }
