...
public class TabContainerDriver<T> : ControlDriver<T> where T : TabContainer
{
  ...
  /// </summary>
  /// <param name="producer">
  /// Producer that creates a TabContainer subclass.
  /// </param>
  /// <param name="description">Driver description.</param>
  public TabContainerDriver(Func<T> producer, string description = "")
  ...
  /// </summary>
  /// <remarks>
  /// Note that Godot requires one additional frame to set visibility of
  /// contents.
  /// </remarks>
  ...
  public void SelectTabWithTitle(string title)
  {
    ...
    if (index < 0)
    {
      throw new ArgumentException(
        $"No tab with the title '{title}' was found.",
        nameof(title)
      );
    }
...
public sealed class TabContainerDriver : TabContainerDriver<TabContainer>
{
  ...
  /// </param>
  /// <param name="description">Driver description.</param>
  public TabContainerDriver(
    Func<TabContainer> producer, string description = ""
  )
    : base(producer, description)
  {
  }
