...
public class GraphNodeDriver<T> : ControlDriver<T> where T : GraphNode
{
  ...
  public GraphNodeDriver(Func<T> producer, string description = "")
  ...
  /// <summary>
  /// Global position of a spot where the node can be safely clicked and
  /// dragged. Will fail if the node is not visible.
  /// </summary>
  protected virtual Vector2 SelectionSpot
  {
    get
    {
      ...
      // we assume a position 5 pixels below the top border, horizontally
      // centered, is a safe selection spot, as the node will have a title
      // there.
  ...
  public void DragBy(float x, float y) => DragBy(new Vector2(x, y));

  /// <summary>
  /// Drags the node by a multiple of its own size multiplied by the given
  /// factor.
  /// </summary>
  /// <param name="delta">Change in distance.</param>
  ...
  /// <summary>
  /// Selects the given node by clicking on it. Same as
  /// <see cref="ClickAtSelectionSpot"/>.
  /// </summary>
  public void Select() => ClickAtSelectionSpot();
  ...
  /// <summary>
  /// Drags a connection from the given source port of this node to the given
  /// target port of the given target node.
  /// </summary>
  /// <param name="sourcePort">Source port.</param>
  /// <param name="targetNode">Target node.</param>
  /// <param name="targetPort">Target node connection port.</param>
  /// <exception cref="ArgumentException"/>
  public void DragConnection(
    Port sourcePort,
    GraphNodeDriver<T> targetNode,
    Port targetPort
  )
  {
    ...
    (
      sourcePort.IsInput &&
      sourcePort.PortIndex >= thisRoot.GetInputPortCount()
    )
    {
      throw new ArgumentException(
        $"Node has no input port at the given index {sourcePort.PortIndex}."
      );
    }
    ...
    (
      sourcePort.IsOutput &&
      sourcePort.PortIndex >= thisRoot.GetOutputPortCount()
    )
    {
      throw new ArgumentException(
        $"Node has no output port at the given index {sourcePort.PortIndex}."
      );
    }
    ...
    (
      targetPort.IsInput &&
      targetPort.PortIndex >= targetRoot.GetInputPortCount()
    )
    {
      throw new ArgumentException(
    ...
    (
      targetPort.IsOutput &&
      targetPort.PortIndex >= targetRoot.GetOutputPortCount()
    )
    {
      throw new ArgumentException(
    ...
    Viewport.DragMouse(
      startPosition + thisRoot.GlobalPosition,
      endPosition + targetRoot.GlobalPosition
    );
  }

  /// <summary>
  /// Drags a connection from the given source port of this node to a position
...
public sealed class GraphNodeDriver : GraphNodeDriver<GraphNode>
{
  ...
  /// <param name="producer">Producer that creates a GraphNode subclass.</param>
  /// <param name="description">Driver description.</param>
  public GraphNodeDriver(Func<GraphNode> producer, string description = "")
    : base(producer, description)
  {
  }
}
