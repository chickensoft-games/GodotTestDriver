...
public class CanvasItemDriver<T> : NodeDriver<T> where T : CanvasItem
{
  ...
  /// </summary>
  /// <param name="producer">
  /// Producer that creates a CanvasItem subclass.
  /// </param>
  /// <param name="description">Driver description.</param>
  public CanvasItemDriver(Func<T> producer, string description = "")
    ...
    get
    {
      ...
      return !root.IsVisibleInTree()
        ? throw new InvalidOperationException(
          ErrorMessage("Cannot interact with CanvasItem because it is not visible.")
        )
        : root;
    }
  }
