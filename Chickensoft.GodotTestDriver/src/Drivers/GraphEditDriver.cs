...
public class GraphEditDriver<TGraphEdit, TGraphNodeDriver, TGraphNode>
  : ControlDriver<TGraphEdit>
    where TGraphEdit : GraphEdit
    where TGraphNode : GraphNode
    where TGraphNodeDriver : GraphNodeDriver<TGraphNode>
{
  private readonly Func<Func<TGraphNode>, string, TGraphNodeDriver>
    _nodeDriverProducer;
  ...
  /// </summary>
  /// <param name="producer">
  /// Producer that produces the <see cref="GraphEdit"/> this driver works on.
  /// </param>
  /// <param name="nodeDriverProducer">
  /// Producer that produces a driver for a <see cref="GraphNode"/> child of the
  /// <see cref="GraphEdit"/>.
  /// </param>
  ...
  /// <summary>
  /// Checks if the graph edit has a connection from the given node to the given
  /// target node on the given ports.
  /// </summary>
  ...
  /// <summary>
  /// Checks if he graph edit has a connection originating from the given node
  /// on the given port.
  /// </summary>
  /// <param name="from">Source GraphNodeDriver.</param>
  /// <param name="fromPort">Source port.</param>
...
/// Driver for a <see cref="GraphEdit"/>.
/// </summary>
public class GraphEditDriver
  : GraphEditDriver<GraphEdit, GraphNodeDriver, GraphNode>
{
  /// <summary>
  /// Constructs a new GraphEdit driver.
  /// </summary>
  ...
  public GraphEditDriver(Func<GraphEdit> producer, string description = "")
    : base(
        producer,
        (node, nodeDescription) =>
          new GraphNodeDriver(node, $"{description}-> {nodeDescription}"),
        description
      )
  {
  }
