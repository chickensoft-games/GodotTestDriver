...
public class Camera2DDriver<T> : Node2DDriver<T> where T : Camera2D
{
  ...
  public Camera2DDriver(Func<T> producer, string description = "")
    : base(producer, description)
  {
  }

  /// <summary>
  /// Starts moving the given position into the view of the camera. This does
  /// NOT wait for the camera to finish moving.
  /// </summary>
  ...
  /// <summary>
  /// Moves the given position into the view of the camera. This will wait for
  /// the given amount of seconds until the camera no longer moves.
  /// </summary>
  /// <param name="worldPosition">World position.</param>
  /// <param name="timeoutSeconds">Timeout, in seconds.</param>
  public async Task<bool> MoveIntoView(
...
public sealed class Camera2DDriver : Camera2DDriver<Camera2D>
{
  ...
  /// <param name="producer">Producer that creates a Camera2D subclass.</param>
  /// <param name="description">Driver description.</param>
  public Camera2DDriver(Func<Camera2D> producer, string description = "")
    : base(producer, description)
  {
  }
}
