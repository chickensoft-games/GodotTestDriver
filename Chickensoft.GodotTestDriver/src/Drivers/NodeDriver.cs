...
/// <summary>
/// Base class for test drivers that work on nodes. This is the non-generic
/// variant, which will make it easier to create some fluent APIs.
/// </summary>
...
public abstract class NodeDriver<T> : NodeDriver where T : Node
{
  ...
  /// <summary>
  /// Builds drivers for a set of children of the current driver's root node.
  /// The first function needs to return the currently applicable child nodes,
  /// the second function will produce a driver for each child.
  /// </summary>
  ...
  protected IEnumerable<TDriver> BuildDrivers<TDriver, TNode>(
    Func<T, IEnumerable<TNode>> childSelector,
    Func<Func<TNode>, TDriver> driverFactory
  )
    where TDriver : NodeDriver
    where TNode : Node
  {
    var root = Root;
    if (root == null)
  ...
  /// <summary>
  /// Helper function to build an error message. Prefixes the message with a
  ...
  /// node is not currently present in the scene tree or if the root node is not
  /// a valid instance anymore.
  /// </summary>
  public T? Root
  {
    get
  ...
  public T PresentRoot => Root ?? throw new InvalidOperationException(
    ErrorMessage("Node is not present in the scene tree.")
  );

  /// <summary>
  /// Returns whether this node has the given signal connected to the given
  /// target.
  /// </summary>
  ...
  public bool IsSignalConnected(
    ...
    PresentRoot.IsConnected(signal, new Callable(target, method));

  /// <summary>
  /// Returns whether the given signal of this node is connected to any other
  /// node.
  /// </summary>
  /// <param name="signal">Signal name.</param>
  public bool IsSignalConnectedToAnywhere(string signal) =>
    PresentRoot.GetSignalConnectionList(signal).Count > 0;
  ...
  public SignalAwaiter GetSignalAwaiter(StringName signal) =>
    PresentRoot.ToSignal(PresentRoot, signal);
}
