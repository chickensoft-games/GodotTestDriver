...
public class ControlDriver<T> : CanvasItemDriver<T> where T : Control
{
  ...
  public ControlDriver(Func<T> producer, string description = "")
  ...
  public bool IsFullyInView
  {
    get
    {
      ...
      var screenRect = control
        .GetViewport()
        .WorldToScreen(control.GetGlobalRect());
  ...
  public virtual void ClickCenter(MouseButton button = MouseButton.Left)
  {
    ...
    control
      .GetViewport()
      .ClickMouseAt(control.GetGlobalRect().Center(), button);
  }

  /// <summary>
  /// Moves the mouse to the center of the control and hovers for the given
  /// time in seconds.
  /// </summary>
