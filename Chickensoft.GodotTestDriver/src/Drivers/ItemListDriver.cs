...
public class ItemListDriver<T> : ControlDriver<T> where T : ItemList
{
  ...
  public ItemListDriver(Func<T> producer, string description = "")
  ...
  public List<string> SelectedItems
  {
    get
    {
      var uiControl = PresentRoot;
      return
        [.. uiControl.GetSelectedItems().Select(i => uiControl.GetItemText(i))];
    }
  }
  ...
  private int IndexOf(string text)
  {
    ...
    throw new InvalidOperationException(
      ErrorMessage($"List does not contain item with text '{text}'.")
    );
  }
  ...
  /// True to add to the current selection, false to change the selection.
  /// </param>
  /// <exception cref="InvalidOperationException"/>
  public void SelectItemWithText(string text, bool addToSelection = false)
  {
    ...
    {
      throw new InvalidOperationException(
        ErrorMessage("Cannot select multiple items because selection mode is not allowing it.")
      );
    }
    ...
    // ourselves if the item was already selected before then only emit the
    // signal if AllowReselect is true
...
public sealed class ItemListDriver : ItemListDriver<ItemList>
{
  ...
  /// <param name="producer">Producer that creates a ItemList subclass.</param>
  /// <param name="description">Driver description.</param>
  public ItemListDriver(Func<ItemList> producer, string description = "")
    : base(producer, description)
  {
  }
}
